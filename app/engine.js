...
function cardOptions(m, card, p, gold, reachable, spaces, spare = 0) {
  ...
  /* A MEASURING SEAT that has sworn off fighting. Not a weight: making attacks
   * merely unattractive pushes every such card below CASH_THRESHOLD, so the
   * seat cashes it for a coin and is PAID for its pacifism — which under the
   * lean economy is a large gift and turns the measurement into a test of
   * cashing rather than of peace. Removing the option instead leaves the card
   * to be spent on the next real thing the bot wanted to do.
   *
   * NOTE `p` here is a SEAT NUMBER, not a Player — every caller passes `p.i`.
   * The set therefore lives on the map, which is the only thing in scope.
   * Never set in a played game; this is for the bench only. */
  return (m.pacifists && m.pacifists.has(p))
    ? out.filter(([, a]) => a !== "attack") : out;
}
...
const TUNED = {
  ...
  /* READING THE TABLE. All four are ZERO in the tuned baseline, and that is
   * not a placeholder - it is what makes every measurement taken before they
   * existed still reproduce exactly. Switch them on with a style.
   *
   * They exist because the bot was, until now, blind to the two things a
   * person at the table cannot help seeing: who is winning, and what shape
   * everyone else is trying to build. A table of bots therefore never ganged
   * up on a leader and never broke a rival's arrangement, which flattered any
   * seat that built quietly and never threw a punch. Any number measured with
   * these at zero is a number from a world where nobody is watching. */
  LEADER_W: 0,             // hit the seat that is AHEAD of you, not the nearest
  OBJ_BREAK_W: 0,          // ...and emptying a tile that is holding up their pattern
  OBJ_HOLD_W: 0,           // put units where they build YOUR arrangement
  OBJ_GUARD_W: 0,          // ...and wall the tiles that are already holding one
};
...
const BOT_STYLES = {
  ...
  /* THE ONE THAT WATCHES THE TABLE. Not a sixth flavour so much as the first
   * bot that plays the social game: it hits whoever is ahead, empties the tile
   * propping up a rival's face-up pattern, builds its own arrangement on
   * purpose and walls it once it stands.
   *
   * Every other style here is a preference between things the bot could
   * already see. This one adds things it could not see at all, which is why it
   * is the style a table should be measured with before anyone concludes that
   * a peaceful seat does well: against the other five, a player who never
   * fights is never punished for it, because nobody is looking. */
  tactician: {
    label: "Tactician",
    note: "plays the table: hits the leader, breaks patterns, builds its own",
    /* LEADER_W is 3 rather than the 1.2 first tried: at 1.2 the style aimed
     * 69% of its blows upward against a baseline of 60%, which is a real
     * difference but sits inside the fingerprint test's tolerance — a policy
     * nobody at the table could reliably notice is not yet a policy. At 3 it
     * aims 76% upward, and mean score is unchanged (26.4 against 26.7), so the
     * style reaches for something different without playing worse. */
    w: { LEADER_W: 3, OBJ_BREAK_W: 0.5, OBJ_HOLD_W: 0.9, OBJ_GUARD_W: 1.5 },
  },
};
const STYLE_KEYS = Object.keys(BOT_STYLES);

/* Styles the MIXED bag never deals. A mixed table is the app's default and the
 * shape most measurements were taken on, so a style that joins the bag quietly
 * changes both. The Tactician is kept out of it deliberately: it is a
 * measuring instrument first, and letting it into every table by default would
 * be a change to what a player faces, made by accident. Choose it by name. */
const MIXED_EXCLUDE = ["tactician"];
...
function valueCard(game, p, k, card, act) {
  ...
  if (act === "settle") {
    ...
    /* AND WHETHER IT BUILDS THE SHAPE I AM AFTER. Both of my own objectives —
     * I may look at my own face-down card, and a person does. Scored on the
     * best of them rather than the sum, so a cell that happens to serve two
     * patterns is not double counted into a decision it should not dominate. */
    if (w.OBJ_HOLD_W) {
      let gain = 0;
      for (const o of p.objectives || [])
        gain = Math.max(gain, game.objectiveNear(p.i, k, o));
      v += w.OBJ_HOLD_W * gain;
    }
    return v;
  }
  ...
  if (act === "attack") {
    const t = m.tiles.get(k);
    /* WHO this is, not just what it costs — zero unless a style asks for it. */
    const bias = game.targetBias(p, t, w);
    if (game.COMBAT === "duel") return duelValue(game, p, t, w, card, bias);
...
function duelValue(game, p, t, w, card, bias = 0) {
  ...
  if (game.SPOILS === "gold") v += SPOILS_V;
    ...
    v += SPOILS_V;
  /* TARGETING RIDES ON THE ODDS. Hitting the leader is worth more than hitting
   * the seat beside you, but not if you cannot win the duel — an attack you
   * will lose helps the leader rather than hurting them. So the bias goes in
   * here, with the spoils, and not onto the total afterwards. */
  v += bias;
...
class Game {
  constructor(n, seed, opts) {
     ...
     *
     *   "reserve" - printed: it goes back into the reserve and may be placed
     *               again with any later card. Measured, this is why combat
     *               does not pay: an attack spends a card NOW for a 57% chance
     *               of making the victim spend one LATER, and because returns
     *               fill the reserve from the top band down while band() reads
     *               the lowest non-empty band, a kill usually does not even
     *               set the victim back on the ladder. A seat forbidden to
     *               attack outscores one that fights even at TWO players,
     *               where every point denied is a point gained.
     *   "out"     - the unit is gone for good. A kill becomes a permanent
     *               point rather than a delay, which is the smallest change
     *               that could make a fight worth its card.
     */
    this.KILLS = opts.kills === "out" ? "out" : "reserve";
  ...
  /* `_quiet` is set while a bot reads the table through score(): a speculative
   * read must not show up in the statistics as though it had happened. */
  inc(k, n) {
    if (this._quiet) return;
    this.stats[k] = (this.stats[k] || 0) + (n === undefined ? 1 : n);
  }
  ...
  *_mulliganPhase() {
  ...
  _dealStyles() {
    const opts = STYLE_KEYS.filter((k) => k !== "tuned" && !MIXED_EXCLUDE.includes(k));
    let bag = [];
    this.P.forEach((p, i) => {
   ...
   * READING THE TABLE
   *
   * Three things a person sitting at the table knows for free and the bot,
   * until v0.26, did not know at all. Each is public information only: nothing
   * here reads a face-down objective, and `openObjectivesOf` is the gate that
   * keeps it honest. A bot that peeked would measure a game nobody plays.
   * ---------------------------------------------------------------- */

  /* WHO IS WINNING, right now. Recomputed once a round rather than once a
   * decision - `score()` walks every tile several times, and a person reads
   * the table at about that rate anyway. Returns each seat's running total. */
  standing() {
    if (this._standingAt !== this.round) {
      this._standingAt = this.round;
      this._quiet = true;
      try { this._standing = this.score().map((r) => r.total); }
      finally { this._quiet = false; }
    }
    return this._standing;
  }

  /* How far AHEAD of me this seat is, as a fraction of the spread across the
   * table: +1 the runaway leader, -1 the seat furthest behind me, 0 level or
   * a table with no spread at all. Normalising by the spread rather than by
   * raw points keeps the term meaningful in round 2 and in round 12, when the
   * same 3-point gap means quite different things. */
  relStanding(me, them) {
    const s = this.standing();
    const spread = Math.max(...s) - Math.min(...s);
    if (!spread) return 0;
    return Math.max(-1, Math.min(1, (s[them] - s[me]) / spread));
  }

  /* The objectives of `seat` that the rest of the table is ALLOWED to see.
   * Open: the two shared cards. Showone (v0.26 printed): the one face up -
   * this is the information that being shown actually costs you, and until
   * now nothing in the engine ever spent it. Secret/both: nothing. */
  openObjectivesOf(seat) {
    if (this.OBJECTIVES_MODE === "open") return this.objectives || [];
    if (this.OBJECTIVES_MODE === "showone") {
      const o = this.P[seat] && this.P[seat].objOpen;
      return o ? [o] : [];
    }
    return [];
  }

  /* Does cell `k` advance this pattern for this seat? 2 = occupying it
   * completes an arrangement, 1 = it is one of the three and the rest is not
   * there yet, 0 = it does nothing. Asked WITHOUT touching the map: the cell
   * is tested as a middle, then as an end beside a middle already held. */
  objectiveNear(seat, k, o) {
    const t = this.m.tiles.get(k);
    if (!t || !o) return 0;
    const mine = (u) => u.owner === seat && u.units.length;
    let best = 0;
    if (t.terrain === o.mid) {                       // k as the middle
      const ends = t.neighbours().filter(mine);
      best = Math.max(best, (ends.some((x) => x.terrain === o.a) ? 1 : 0)
                          + (ends.some((x) => x.terrain === o.b) ? 1 : 0));
    }
    for (const u of t.neighbours()) {                // k as an end
      if (u.terrain !== o.mid || !mine(u)) continue;
      const ends = u.neighbours().filter(mine).filter((x) => x !== t);
      const needA = t.terrain === o.a && !ends.some((x) => x.terrain === o.a);
      const needB = t.terrain === o.b && !ends.some((x) => x.terrain === o.b);
      if (!needA && !needB) continue;
      const other = needA ? ends.some((x) => x.terrain === o.b)
                          : ends.some((x) => x.terrain === o.a);
      best = Math.max(best, other ? 2 : 1);
    }
    return best;
  }

  /* Is this tile of mine currently HOLDING UP an arrangement? Used to decide
   * what to wall, and - read against a rival, through their open card only -
   * what is worth emptying. */
  objectiveTiles(seat, objs) {
    const out = new Set();
    for (const o of objs || []) for (const trio of this.objectiveHits(seat, o))
      for (const key of trio) out.add(key);
    return out;
  }

  /* What targeting adds to the value of attacking this tile. Zero unless a
   * style has switched it on, and deliberately a single method so that both
   * combat rules get the same bias and neither can drift from the other. */
  targetBias(p, t, w) {
    if (t.owner === null || t.owner === p.i) return 0;
    let v = 0;
    if (w.LEADER_W) v += w.LEADER_W * this.relStanding(p.i, t.owner);
    /* Breaking a pattern needs the tile EMPTIED, not merely bloodied - a stack
     * of two goes on holding the arrangement after it loses one unit. So this
     * rides on the same condition as ATTACK_LONE_V, and is worth what the
     * arrangement is worth under whichever scoring rule is on. */
    if (w.OBJ_BREAK_W && t.units.length === 1) {
      const open = this.openObjectivesOf(t.owner);
      if (open.length && this.objectiveTiles(t.owner, open).has(t.key))
        v += w.OBJ_BREAK_W * (this.OBJ_SCORING === "perInstance" ? this.OBJ_PER : 1);
    }
    return v;
  }
      ...
      switch (ans.kind) {
        ...
        case "conquest": {
          ...
          for (let n = 0; n < kills; n++) {
            ...
            if (this.KILLS === "reserve") this.P[victim].returnUnit();
            else this.inc("killed_for_good");
  ...
  *_duel(p, cell, tile, attack, fort) {
    ...
    this.inc("duels");
    /* WHERE THE BLOW LANDED, relative to the table. Purely observational — no
     * rule reads it — but it is the only dimension on which a targeting style
     * differs from the baseline at all, and without it the style fingerprint
     * in bots_test.js cannot tell a Tactician from a Balanced bot. */
    {
      const st = this.standing();
      const mid = (Math.max(...st) + Math.min(...st)) / 2;
      const def = d ? d.i : tile.owner;
      if (def !== null && def !== undefined && st[def] > mid) this.inc("aimed_up");
    }
  ...
  *_freeMoves(p) {
  ...
  /* The tiles of mine currently completing an arrangement, cached for the one
   * pass of fortifyCandidates that asks. */
  _objGuard(p) { return this.objectiveTiles(p.i, p.objectives || []); }
  ...
  fortifyCandidates(p) {
    ...
    for (const [k, t] of this.m.tiles) {
      ...
      /* AND WALL WHAT IS ALREADY SCORING. A tile holding up an arrangement is
       * worth more than its unit: losing it costs the pattern too. Zero unless
       * a style asks for it. Both of my objectives count — it is my own board
       * I am defending, so nothing hidden is being read here. */
      if (p.w.OBJ_GUARD_W && this._objGuard(p).has(k)) v += p.w.OBJ_GUARD_W;
      if (v > 0) out.push([v, k]);
    }
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    TUNED, BOT_STYLES, BOT_LEVELS, STYLE_KEYS, MIXED_EXCLUDE, botWeights,
    setMeldRules, meldRules, meldFault, isRun, isFriends, canCombine, BAND_HOLDS,
    TIER_UNITS, TIER_CAPS, parseLayout, bandsFor, DECK_SPLIT,
